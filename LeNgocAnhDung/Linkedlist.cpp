...
{
	this->readFile();
	for (Node *pTmp = this->pHead; pTmp != NULL; pTmp = pTmp->next)
	{
      for (Node *pTmp2 = pTmp->next; pTmp2 != NULL; pTmp2 = pTmp2->next)
		{
			if (pTmp->pt.getHangSanXuat() < pTmp2->pt.getHangSanXuat())
			{
				swap(&pTmp, &pTmp2);
				cout<<pTmp->pt.getHangSanXuat()<<endl;
				cout<<pTmp2->pt.getHangSanXuat()<<endl;
			}
		}
	}
} */